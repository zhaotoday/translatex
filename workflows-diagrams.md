...
graph TB
    subgraph local["开发者本地环境"]
        A[前端项目 Vue/React] 
        B[CLI 工具 @translatex/cli]
        C[配置文件 translatex.config.js]
        D[语言包目录 locales/]
    end
    
    subgraph platform["翻译管理平台"]
        E[Nginx 负载均衡]
        F[前端应用 Vue 3]
        G[后端 API NestJS]
        H[Queue Worker 后台任务]
...
sequenceDiagram
    ...
    participant CLI as translatex-cli
    participant API as 平台 API
    participant FS as 文件系统
    
    Dev->>CLI: translatex init
    CLI->>Dev: 请输入项目信息
    Dev-->>CLI: 项目名称、框架等
    CLI->>API: POST /api/projects 创建项目
    API-->>CLI: 返回 projectId 和 apiKey
    CLI->>FS: 创建 translatex.config.js
    FS-->>CLI: 配置文件已创建
    CLI->>Dev: ✅ 初始化完成
```
...
```mermaid
flowchart TD
    Start([translatex extract]) --> LoadConfig[加载配置文件]
    LoadConfig --> ScanFiles[扫描源代码文件 根据 include/exclude]
    
    ScanFiles --> CheckFramework{检查框架类型}
    CheckFramework -->|Vue| VueParser[Vue 解析器]
    CheckFramework -->|React| ReactParser[React 解析器]
    
    VueParser --> ParseTemplate[解析 Template 查找 $t 调用]
    VueParser --> ParseScript[解析 Script AST 遍历]
    
    ReactParser --> ParseJSX[解析 JSX 查找 t 和 Trans]
    ...
    Deduplicate --> AddMeta[添加元信息 文件路径、行号]
    AddMeta --> Output[输出到 JSON 文件 locales/extracted/]
...
sequenceDiagram
    ...
    Dev->>CLI: translatex push
    CLI->>FS: 读取 extracted.json
    FS-->>CLI: 词条数据
    
    CLI->>CLI: 验证词条格式
    CLI->>API: POST /api/projects/:id/entries/batch Header: X-API-Key
...
sequenceDiagram
    ...
    Dev->>CLI: translatex pull --locales en-US,ja-JP
    CLI->>CLI: 读取配置 获取 projectId、apiKey
    
    loop 每个语言
        CLI->>API: GET /api/projects/:id/export/:locale ?version=master
        API->>DB: 查询主版本
        API->>DB: 获取该语言所有翻译
        
        DB-->>API: 翻译数据
        API->>API: 组装成语言包 key: value 格式
        ...
        CLI->>FS: 写入文件 src/locales/en-US.json
    end
    
    FS-->>CLI: 文件已保存
    CLI->>Dev: ✅ 下载完成 en-US.json, ja-JP.json
```
...
```mermaid
flowchart TD
    Start([用户点击批量 AI 翻译]) --> SelectLocales[选择目标语言 en-US, ja-JP]
    SelectLocales --> CreateJobs[创建翻译任务 translation_jobs]
    
    CreateJobs --> Job1[Job 1: zh-CN to en-US]
    CreateJobs --> Job2[Job 2: zh-CN to ja-JP]
...
sequenceDiagram
    ...
    User->>Web: 点击批量 AI 翻译
    Web->>API: POST /api/projects/:id/translations/ai-translate
    
    API->>DB: 创建 translation_jobs 记录
    DB-->>API: job_ids: [1, 2]
    
    API->>Queue: publish translation
    API->>Queue: publish translation
    ...
    Worker->>Web: WebSocket 通知前端
    Web->>User: 更新 UI：翻译中...
    
    Worker->>DB: SELECT entries WHERE project_id=?
    DB-->>Worker: entry1, entry2, ..., entry100
    
    loop 分批处理 每批 20 条
        Worker->>OpenAI: POST /chat/completions
        OpenAI-->>Worker: translation1, translation2, ...
        
        Worker->>DB: INSERT INTO translations
...
```mermaid
flowchart TD
    Start([开发者创建版本]) --> CLI[translatex version:create v1.0.0]
    
    CLI --> GetBranch[获取当前 Git 分支]
    GetBranch --> GetCommit[获取最新 commit hash]
    
    GetCommit --> CallAPI[POST /api/projects/:id/versions]
    
    CallAPI --> QueryEntries[查询当前所有词条]
    QueryEntries --> QueryTranslations[查询所有翻译]
    
    QueryTranslations --> CreateSnapshot[创建快照 保存词条和翻译状态]
    
    CreateSnapshot --> InsertVersion[INSERT INTO versions]
    
    InsertVersion --> CreateAssociations[创建词条版本关联 entry_versions 表]
    
    CreateAssociations --> Response[返回版本信息]
    Response --> CLI
    
    CLI --> Success([✅ 版本创建成功 v1.0.0])
```
...
sequenceDiagram
    ...
    Dev->>CLI: translatex version:master v1.0.0
    CLI->>API: POST /api/versions/:id/set-master
    
    API->>DB: BEGIN TRANSACTION
    
    API->>DB: UPDATE versions SET is_master=false
    
    API->>DB: UPDATE versions SET is_master=true WHERE version_name=v1.0.0
...
```mermaid
flowchart LR
    subgraph v1["版本 v1.0.0"]
        V1E1[home.welcome: 欢迎]
        V1E2[home.title: 首页]
        V1E3[user.name: 用户名]
    end
    
    subgraph v2["版本 v1.1.0"]
...
graph LR
    ...
    B -->|push| C[平台数据库 entries 表]
    C -->|AI/手动翻译| D[translations 表]
    D -->|创建版本| E[entry_versions 表 快照]
    E -->|pull| F[本地语言包 locales/*.json]
    F -->|加载| G[前端应用 运行时]
```
...
flowchart TD
    ...
    Auth --> GetVersion[获取版本信息 默认 master]
    
    GetVersion --> QueryData[查询该版本的 所有词条和翻译]
    
    QueryData --> Loop{遍历每个语言}
    
    Loop -->|en-US| BuildEN[构建 en-US.json]
    ...
    AddToZip --> CreateZip[创建 ZIP 文件 project-v1.0.0.zip]
    
    CreateZip --> Upload[上传到 OSS 可选]
...
flowchart TD
    ...
    CheckError -->|速率限制| Retry[等待后重试 最多 3 次]
    CheckError -->|网络错误| Retry
    CheckError -->|API 错误| LogError[记录错误日志]
    
    Retry --> RetryCount{重试次数?}
    RetryCount -->|小于3| Try
    RetryCount -->|大于等于3| MarkFailed[标记任务失败]
    
    LogError --> MarkFailed
    MarkFailed --> UpdateJob[更新 job status=failed]
    
    UpdateJob --> NotifyUser[通知用户 WebSocket/邮件]
...
sequenceDiagram
    ...
    Worker->>Redis: PUBLISH translation:progress
    ...
    Note over Worker,WS: 类似的事件：entry:created entry:updated translation:completed version:created
```
